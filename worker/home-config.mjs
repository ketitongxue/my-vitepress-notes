...
function parseStoredConfig(value) {
  ...
  try {
    ...
    throw new HomeConfigError('Stored configuration is not valid JSON')
  }
...
function versionRecord(row) {
  ...
  try {
    ...
    if (!(caught instanceof HomeConfigError)) throw caught
    record.validationError = caught.message
  }
  return record
}

export async function handlePublicHomeConfig(request, env) {
  ...
  try {
    const row = await requireDatabase(env).prepare(`
      ...
      LIMIT 1
    `).first()
    if (!row) return error('HOME_CONFIG_UNAVAILABLE', 503)
    const config = parseStoredConfig(row.config_json)
    const etag = `"home-${row.revision}"`
    ...
    return error('HOME_CONFIG_UNAVAILABLE', 503)
  }
}
...
export function createHomeAdminHandler({ authenticate = authenticateAdmin } = {}) {
  return async function handleHomeAdmin(request, env) {
    ...
    try {
      ...
      return error('HOME_CONFIG_DB_UNAVAILABLE', 503)
    }

    if (request.method === 'GET' && pathname === '/api/admin/home/config') {
      try {
    ...
    if (request.method === 'PUT' && pathname === '/api/admin/home/config') {
      ...
      try {
        ...
        if (schemaVersion !== HOME_CONFIG_SCHEMA_VERSION) return error('UNSUPPORTED_SCHEMA_VERSION', 400)
        ...
        const config = normalizeHomeConfig(body.value?.config)
        const result = await db.prepare(`
          INSERT INTO home_config_versions
            (schema_version, config_json, note, created_by)
          SELECT ?, ?, ?, ?
          WHERE ? = (SELECT COALESCE(MAX(revision), 0) FROM home_config_versions)
        `).bind(
          schemaVersion,
        ...
        if (caught instanceof HomeConfigError) return error('INVALID_CONFIG', 400, caught.message)
        return error('HOME_CONFIG_DB_UNAVAILABLE', 503)
      }
    }

    if (request.method === 'POST' && pathname === '/api/admin/home/publish') {
      const originError = verifyMutationOrigin(request, env)
      if (originError) return originError
      const body = await readJson(request)
      if (!body.ok) return body.response
      ...
      try {
        const row = await db.prepare(`
          ...
          FROM home_config_versions
          WHERE revision = ?
            AND revision = (SELECT MAX(revision) FROM home_config_versions)
        `).bind(revision).first()
        if (!row) return error('REVISION_CONFLICT', 409)
        parseStoredConfig(row.config_json)
        const result = await db.prepare(`
          UPDATE home_config_versions
    ...
    if (request.method === 'POST' && pathname === '/api/admin/home/rollback') {
      const originError = verifyMutationOrigin(request, env)
      if (originError) return originError
      const body = await readJson(request)
      if (!body.ok) return body.response
...
export const handleHomeAdmin = createHomeAdminHandler()
