...
test('desktop components use local Tabler icons and native pointer interactions', () => {
  ...
  assert.doesNotMatch(surface, /configuration\.desktop\.resetLabel|resetIconPositions|重置桌面位置/)
  assert.match(surface, /height: 40px/)
  assert.match(surface, /configuration\.desktop\.brand/)
  assert.match(surface, /v-for="link in configuration\.desktop\.menuLinks"/)
...
test('canvas persistence uses only complete trusted revision-aware v3 geometry', () => {
  ...
  const lastCardIndex = defaults.cards.length - 1
  assert.equal(parsed.cards[lastCardIndex].x, defaults.cards[lastCardIndex].x + lastCardIndex)
...
test('dynamic bounds measure visible cards and share one Fit rectangle', () => {
  ...
  assert.equal(initialFitCards(canvasCards, false).length, 8)
...
test('system canvas is read-only content with alternate navigation paths', () => {
  ...
  assert.match(sources, /aria-label="AI 纪元无限画布"/)
