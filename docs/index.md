---
layout: home

hero:
  name: AI 纪元
  text: 探索智能时代的知识、工具与创造
  tagline: 持续记录 AI、产品、工程与个人实践之间的连接。
  actions:
    - theme: brand
      text: 浏览知识库
      link: /wiki/
    - theme: alt
      text: 向知识库提问
      link: /ask/

features:
  - icon: '<span aria-hidden="true">◇</span>'
    title: AI 与模型
    details: 原理、工具与工作流
    link: /notes/sustainable-ai-workflow
    linkText: 查看专题
  - icon: '<span aria-hidden="true">△</span>'
    title: 产品与设计
    details: 验证、体验与迭代
    link: /notes/product-validation-loop
    linkText: 查看专题
  - icon: '<span aria-hidden="true">○</span>'
    title: 工程实践
    details: 构建、部署与自动化
    link: /notes/static-site-delivery
    linkText: 查看专题
---

<section class="garden-section">
  <div>
    <p class="garden-eyebrow">最新内容</p>
    <h2>最近更新</h2>
    <ul class="garden-list">
      <li><a href="/finance/">金融知识库</a><time datetime="2026-07-08">2026-07-08</time></li>
      <li><a href="/wiki/">AI 知识库</a><time datetime="2026-07-07">2026-07-07</time></li>
      <li><a href="/ask/">知识库问答</a><time datetime="2026-07-07">2026-07-07</time></li>
    </ul>
  </div>
  <aside>
    <p class="garden-eyebrow">站内导航</p>
    <h2>快速入口</h2>
    <nav class="garden-links" aria-label="站内快速入口">
      <a href="/wiki/">AI 知识库</a>
      <a href="/finance/">金融知识库</a>
      <a href="/ask/">知识库问答</a>
      <a href="/llm-wiki/">LLM Wiki Skill</a>
      <a href="/notes/sustainable-ai-workflow">精选笔记</a>
    </nav>
  </aside>
</section>
