...
同一习惯也支持商业验证：不直接检验核心需求的基础设施应复用现成方案，把定制工作留给必须验证的产品洞察或工作流。[AI 产品设计润色工作流](/wiki/concepts/ai-product-design-polish-workflow)把这套发现习惯用于设计：组件库、模板画廊和成熟产品都可作为参考；应逐段检查和改造，而非要求模型从空白提示发明视觉系统。
