...
<template>
  <main class="os-admin">
    ...
    <p v-if="invalidVersions.length" class="os-admin__error" role="status">
      ...
      <aside class="os-admin__versions" aria-labelledby="version-title">
        ...
        <ol v-else>
          <li v-for="version in versions" :key="version.revision">
            ...
            <p v-if="version.validationError" class="os-admin__invalid-version">配置无效：{{ version.validationError }}</p>
...
.os-admin__versions p.os-admin__invalid-version { color: #9b3e23; overflow-wrap: anywhere; }
