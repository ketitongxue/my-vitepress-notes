...
go-tiny-claw 目前更像一个 Agent runtime 实验场：从一次用户指令开始，模型在执行循环中决定是否调用工具；工具结果再回到上下文，推动下一轮行动，直到任务结束。这样的结构让我可以分别观察<mark class="reading-mark">模型、工具、上下文和外部系统之间的边界</mark>，而不是把所有逻辑塞进一个巨大的 handler。
...
<ReadingCode filename="go-tiny-claw / 目录结构示意" language="text">
...
<ReadingSteps label="Agent 单轮执行流程示意">
  <li><strong>组装上下文</strong><p>汇集系统提示和会话中的工作记忆。</p></li>
  <li><strong>调用模型</strong><p>获得文本回复或本轮工具调用。</p></li>
  <li><strong>执行工具</strong><p>有工具调用时，并发执行同一轮工具。</p></li>
  <li><strong>写回观察</strong><p>把工具结果交回会话，供后续循环使用。</p></li>
</ReadingSteps>
...
<ReadingInsight title="工具边界是设计的一部分">
...
<ReadingInsight title="当前集成状态" tone="warning">
...
<ReadingComparison label="Agent 工程设计取舍" before-title="需要避免" after-title="对应的工程约束">
<template #before>
